...
int compter_points_route(Plateau* p, int x, int y, int visite[TAILLE_MAX][TAILLE_MAX]) {
    ...
    // carrefour = fin de route, on compte la tuile mais on suit pas les autre routes
    if (t.center == CARREFOUR) return 1;
...
// verifie si une route est complete - version corrigee qui gere les carrefour
static int route_terminee_rec(Plateau* p, int x, int y, int visite[TAILLE_MAX][TAILLE_MAX]) {
    if (x < 0 || x >= TAILLE_MAX || y < 0 || y >= TAILLE_MAX || p->occupes[x][y] == 0)
        return 0;
    if (visite[x][y]) return 1;
    visite[x][y] = 1;
    Tuiles t = p->grille[x][y];

    // carrefour ferme la route de ce cote la, on s'arrete
    if (t.center == CARREFOUR) return 1;

    if (t.a == ROUTE_PRAIRIE) {
        if (y + 1 >= TAILLE_MAX || !p->occupes[x][y+1] || p->grille[x][y+1].c != ROUTE_PRAIRIE) return 0;
        if (!route_terminee_rec(p, x, y+1, visite)) return 0;
    }
    if (t.b == ROUTE_PRAIRIE) {
        if (x + 1 >= TAILLE_MAX || !p->occupes[x+1][y] || p->grille[x+1][y].d != ROUTE_PRAIRIE) return 0;
        if (!route_terminee_rec(p, x+1, y, visite)) return 0;
    }
    if (t.c == ROUTE_PRAIRIE) {
        if (y - 1 < 0 || !p->occupes[x][y-1] || p->grille[x][y-1].a != ROUTE_PRAIRIE) return 0;
        if (!route_terminee_rec(p, x, y-1, visite)) return 0;
    }
    if (t.d == ROUTE_PRAIRIE) {
        if (x - 1 < 0 || !p->occupes[x-1][y] || p->grille[x-1][y].b != ROUTE_PRAIRIE) return 0;
        if (!route_terminee_rec(p, x-1, y, visite)) return 0;
    }
    return 1;
}

static int verifie_route(Plateau* p, int x, int y) {
    int visite[TAILLE_MAX][TAILLE_MAX] = {0};
    return route_terminee_rec(p, x, y, visite);
}
...
void verifier_et_scorer_structures(Plateau* p, int x, int y, Joueur joueurs[], int nb_joueurs) {
    ...
    // 3. Vérification Route (on utilise notre propre check qui gere bien les carrefour)
    if (t.a == ROUTE_PRAIRIE || t.b == ROUTE_PRAIRIE || t.c == ROUTE_PRAIRIE || t.d == ROUTE_PRAIRIE) {
        if (verifie_route(p, x, y) == 1) {
            int visite[TAILLE_MAX][TAILLE_MAX] = {0};
